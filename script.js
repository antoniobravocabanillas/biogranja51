// Lista manual de TikTok URLs (pon las tuyas aquí)
const TIKTOK_URLS = [
  "https://www.tiktok.com/@tu_usuario/video/7123456789012345678",
  "https://www.tiktok.com/@tu_usuario/video/7123456789012345679"
];

// Helper: extraer ID y construir embed iframe src
function tiktokEmbedSrc(url) {
  // TikTok embed iframe formato moderno:
...
async function buildTiktokGrid(selector) {
  const container = document.querySelector(selector);
  if (!container) return;

  for (const url of TIKTOK_URLS) {
    // Obtén miniatura via oEmbed (puede fallar por CORS, pero muchos funcionan)
    let thumb = "";
    try {
      const oembedUrl = `https://www.tiktok.com/oembed?url=${encodeURIComponent(url)}`;
      const res = await fetch(oembedUrl); // esto puede petar por CORS en algunos dominios
      if (res.ok) {
        const data = await res.json();
        thumb = data.thumbnail_url || "";
      }
    } catch (err) {
      console.warn("oEmbed falló (CORS?), continuamos sin thumb", err);
    }

    // Si no hay miniatura, usa placeholder
    if (!thumb) thumb = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(
      `<svg xmlns='http://www.w3.org/2000/svg' width='600' height='400'><rect width='100%' height='100%' fill='#eee'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='#999' font-size='20'>TikTok</text></svg>`
    );

    const card = document.createElement("div");
    card.className = "tiktok-card";
    card.innerHTML = `
      <img class="tiktok-thumb" src="${thumb}" loading="lazy" alt="TikTok preview">
      <div class="tiktok-meta">Ver video</div>
    `;
    container.appendChild(card);

    const embedSrc = tiktokEmbedSrc(url);
...
function openTiktokModal(embedSrc) {
  if (!embedSrc) { alert("URL de TikTok inválida."); return; }
  const modal = document.getElementById("tiktok-modal");
  const content = document.getElementById("modal-content");
  content.innerHTML = `<iframe src="${embedSrc}" style="width:100%;height:600px;border:0;" allow="autoplay; encrypted-media" allowfullscreen></iframe>`;
...
// Inicializar
buildTiktokGrid("#tiktok-section");
